...
install_libraries() {
    ...
    required_libs=("AsyncTCP" "ESPAsyncWebServer" "Servo")
...
generate_code() {
    ...
    cat <<EOF >esp32_code/esp32_code.ino
#include <WiFi.h>
#include <AsyncTCP.h>
...
upload_code() {
    ...
    local port="$1"
    arduino-cli compile --fqbn esp32:esp32:esp32 esp32_code
    if [ $? -ne 0 ]; then
    ...
    arduino-cli upload -p "$port" --fqbn esp32:esp32:esp32 esp32_code
    if [ $? -ne 0 ]; then
